...
- `tasks/`：各阶段任务简报与验证报告。
- `docs/subagent-prompt.md`：子代理派发通用任务书（多代理并行时作为前缀逐任务实例化）。
...
- **Spring AI 流式工具调用丢请求上下文（R1 发现，2026-09-05 已修+实测闭环）**：`.stream()` 时 @Tool 在 Reactor boundedElastic 线程执行，RequestContextHolder（Tomcat 线程 ThreadLocal）为空。已修：chat() 入口经 `PatientChatGuard` 校验 PATIENT 角色并把 X-User-Id 解析为 patient_archive.id（sys_user id 与档案 id 是独立自增序列，**不可直传**，撞车即越权），以 `forPatient` 不可变快照绑定工具视图；T4 真实链路回归已通过（2026-09-05 DeepSeek 实网，读/写两路径零 401，tasks/T4-流式工具回归-验证报告.md）。同源陷阱：`HeaderRelayInterceptor` 也读 RequestContextHolder，工具线程上的 Feign 不转发 X-User-Id/X-Trace-Id（内部契约端点不读头不受影响，仅断链路追踪）。
- **多构造器 bean 必须显式 @Autowired（T4 实测，2026-09-05）**：@Component 类一旦出现第二个构造器（如请求级视图的 private 构造器），Spring 不再自动选主构造器而回退无参构造 → 启动即败；纯 Mockito 单测对装配失败零可见（连续两轮"测试全绿、运行期失效"）。缓解：装配冒烟测试（PatientToolsWiringTest 模式：AnnotationConfigApplicationContext + mock bean + refresh）。另：SSE 端点（produces=text/event-stream）抛业务异常时 JSON 错误体协商失败 → 传输层 500 空体——已处置（2026-09-05）：ai 本地 SseBizExceptionHandler——SSE Accept 下返回真实状态码 + text/event-stream 错误事件体。
- **patient_archive.id_card_hash 存量回填**：存量行哈希为 NULL（SQL 无法对 AES 密文算哈希），唯一索引允许多 NULL——回填完成前存量行不参与判重，新建与存量同证号可重复；回填需应用侧解密重哈希并自行处理撞键。
...
- 最近更新：2026-09-05
- 更新原因：R2 修复回写 + T4 真实链路回归闭环（流式身份快照/知识库守卫/建档防重/装配冒烟，陷阱条目同步更新）
- 审阅者：待指导老师/本人确认
